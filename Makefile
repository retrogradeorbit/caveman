...
APP=build/js/compiled/caveman.js
IDX=build/index.html
IMG=build/img/sprites.png build/img/fonts.png build/img/light.png build/img/u.png build/img/h.png build/img/g.png
...
REPO=git@github.com:retrogradeorbit/caveman.git
...
itch.io: all
	rm -rf release caveman.zip
	cp -a build release
	rm -rf release/.git
	cd release && zip -r ../caveman.zip .
